...
$(function(){
	...
	var Datagrid = function (element, options) {
		...
		this.$select = this.$element.find('.grid-pager select');
...
//		this.$pagedropdown = this.$element.find('.grid-pager .dropdown-menu');
		...
		this.$select.on('change', $.proxy(this.pageChanged, this));
	...
	Datagrid.prototype = {
		...
		renderColumns: function () {
			...
			// +1 = coluna do botao
			this.$footer.attr('colspan', this.columns.length+1);
			this.$topheader.attr('colspan', this.columns.length+1);
			...
			$.each(this.columns, function (index, column) {
				...
				if(column.width) 	 colHTML += ' style="width:'+ column.width+ '%"';
		...
		renderData: function () {
			...
			this.options.dataSource.data(this.options.dataOptions, function (data) {
				...
				$.each(data.data, function (index, row) {
					...
					btn.click(function(){
						Leve.return_value(row);
					});
					btn.append('<i class="icon-share"></i>');
					
					$('<td />').appendTo(rowEl).append(btn);
					
					$.each(self.columns, function (index, column) {
						rowEl.append('<td>' + Object.byString(row, column.property) + '</td>');
					});
					self.$tbody.append(rowEl);
				});
				
				if (self.$tbody.find('tr').length == 0) self.$tbody.html(self.placeholderRowHTML('0 ' + self.options.itemsText));

				self.$select.val(data.page);
		...
		placeholderRowHTML: function (content) {
			...
			return '<tr><td style="text-align:center;padding:20px;" colspan="' +
				(this.columns.length+1) + '">' + content + '</td></tr>';
		},
