...
//			alert('Error! implement LeveModule.submitForm in your page');
		...
		loadModule: function(obj){
			if(obj){
				if($.type(obj) == 'object'){
					var id_obj = obj[$('form.leve_module_form').data("id-property")];
					
					$.getJSON($('form.leve_module_form').attr("action")+"/"+id_obj, function(data) {
							if ( !reciveError(data)) {
...
//			alert('Error! implement LeveModule.openModule in your page');
		...
		remove: function(){
			var id_obj = $('#'+$('form.leve_module_form').data("id-property")).val();
			
			$.getJSON($('form.leve_module_form').attr("action")+"/delete/"+id_obj, function(data) {
				if(!data){
					LeveModule.loadModule();
					showSuccessMsg($('form.leve_module_form').data("success-delete-msg"));
				} else {
					!reciveError(data);
				}
...
//			alert('Error! implement LeveModule.lookup_return in your page');
	...
	//limpa
	$('.leve-lookup input').keydown(function (e){ 
		var pattern = /\w/; 
		if(pattern.test( String.fromCharCode(e.which)) || 
							e.keyCode == 8 || 
							e.keyCode == 32|| 
							e.keyCode == 27){
			$(this).parents('.leve-lookup').find('input[id!="'+$(this).attr('id')+'"]').val('');
		} 
		if(e.keyCode == 27){
			$(this).val('') 
		}
	...
	$('.leve-lookup input.leve-lookup-desc').blur(function(){
		if($(this).parents('.leve-lookup').find('input.leve-lookup-id').val() =='' ){
			$(this).val('');
		}
	});
	
	$('.leve-lookup button.leve-lookup-find-btn').click(function(){
		var url = $(this).parents('div.leve-lookup').data('find-url');
		LeveModule.lookup_return = window[$(this).parents('div.leve-lookup').data('function-fill')];
		Leve.openLookup(url);
		return false;
	});
						   
	$('.leve-lookup button.leve-lookup-cad-btn').click(function(){
